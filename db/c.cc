...
void rocksdb_iter_atg_destroy(rocksdb_iterator_atg_t* iter) {
  delete iter->rep;
  delete iter;
}
...
rocksdb_iterator_atg_t* rocksdb_transactiondb_create_iterator_atg(
  ...
  std::unique_ptr<AttributeGroupIterator> iter = txn_db->rep->NewAttributeGroupIterator(options->rep, column_families);
